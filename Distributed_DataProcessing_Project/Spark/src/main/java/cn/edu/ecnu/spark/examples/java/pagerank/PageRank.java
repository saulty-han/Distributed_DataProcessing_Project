...
//    conf.setMaster("local"); // 仅用于本地进行调试，如在集群中运行则删除本行
    ...
    // 读取输入文本
    JavaRDD<String> text = sc.textFile(args[2]);   // "src/main/resources/input/pagerank/pagerank.txt"
...
//    });
      // 获取输出目录路径参数
      String outputPath = args[1]; // 从命令行参数中获取输出路径
      ...
      ranks.mapToPair(
              ...
              .saveAsTextFile(outputPath);  // 输出路径来自命令行参数
